...
auto RandomGenerator::GetRanduint64_t(uint64_t min, uint64_t max) -> uint64_t {
...
auto RandomGenerator::GetRanduint64_t() -> uint64_t { return mt_generator.Rand(); }
...
auto Rand(int n) -> int { return RandomGenerator::GetRand(0, n); }

auto UniformRand(int low, int high) -> int { return Rand(high - low + 1) + low; }

auto UniformRandExcept(int low, int high, int v) -> int {
...
auto NonUniformRand(int a, int x, int y, int c) -> int {
